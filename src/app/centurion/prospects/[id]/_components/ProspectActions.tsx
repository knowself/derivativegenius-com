...
export default function ProspectActions({ prospect, opportunityId }: { prospect: ProspectEvidence; opportunityId?: string }) {
  ...
  return <div className="grid lg:grid-cols-2 gap-5">
    <section className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
      ...
      <div className="grid sm:grid-cols-2 gap-2"><input data-lpignore="true" type="number" value={evidence.reviewCount ?? 0} onChange={(event) => setEvidence({ ...evidence, reviewCount: Number(event.target.value) })} placeholder="Google reviews" className="field" /><input data-lpignore="true" value={evidence.sourceUrl ?? ''} onChange={(event) => setEvidence({ ...evidence, sourceUrl: event.target.value })} placeholder="Evidence source URL" className="field" /></div>
      <textarea data-lpignore="true" value={evidence.websiteObservation ?? ''} onChange={(event) => setEvidence({ ...evidence, websiteObservation: event.target.value })} placeholder="Specific website observation" className="field w-full" />
      <textarea data-lpignore="true" value={evidence.commercialConsequence ?? ''} onChange={(event) => setEvidence({ ...evidence, commercialConsequence: event.target.value })} placeholder="Likely commercial consequence" className="field w-full" />
      <textarea data-lpignore="true" value={evidence.notes ?? ''} onChange={(event) => setEvidence({ ...evidence, notes: event.target.value })} placeholder="Research notes" className="field w-full" />
    ...
    <section className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
      ...
      <div className="grid sm:grid-cols-2 gap-2"><input data-lpignore="true" value={contact.fullName} onChange={(event) => setContact({ ...contact, fullName: event.target.value })} placeholder="Full name" className="field" /><input data-lpignore="true" value={contact.roleTitle} onChange={(event) => setContact({ ...contact, roleTitle: event.target.value })} placeholder="Role / title" className="field" /><input data-lpignore="true" value={contact.email} onChange={(event) => setContact({ ...contact, email: event.target.value })} placeholder="Business email" className="field" /><input data-lpignore="true" value={contact.phone} onChange={(event) => setContact({ ...contact, phone: event.target.value })} placeholder="Business phone" className="field" /></div>
    ...
    <section className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
      ...
      <div className="grid sm:grid-cols-2 gap-2"><select data-lpignore="true" value={deal.stage} onChange={(event) => setDeal({ ...deal, stage: event.target.value })} className="field"><option>qualified</option><option>discovery_scheduled</option><option>audit_accepted</option><option>proposal_sent</option><option>closed_won</option><option>closed_lost</option></select><input data-lpignore="true" type="number" value={deal.estimatedValue} onChange={(event) => setDeal({ ...deal, estimatedValue: event.target.value })} className="field" /><input data-lpignore="true" type="number" value={deal.probabilityPercent} onChange={(event) => setDeal({ ...deal, probabilityPercent: event.target.value })} className="field" /><input data-lpignore="true" value={deal.packageName} onChange={(event) => setDeal({ ...deal, packageName: event.target.value })} className="field" /></div>
