...
export default function SolutionsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 space-y-16">
            ...
            <Card key={idx} className="flex flex-col justify-between">
              ...
              <CardContent className="space-y-4 pt-0">
                ...
                <div className="pt-2">
                  <Link
                    href="/contact"
                    className="inline-flex items-center space-x-1.5 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300"
                  >
      ...
      <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/80 p-8 space-y-8 shadow-sm">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
          <div>
            <div className="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">Engineering Offerings</div>
            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white">Core Web &amp; AI Services</h2>
          ...
          <Link
            href="/services"
            className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-500 transition shrink-0 active:scale-95"
          >
            Explore All Services <ArrowRight className="w-4 h-4" />
          </Link>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-slate-700 dark:text-slate-300">
          <div className="p-5 rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/60 space-y-3">
            <div className="font-bold text-slate-900 dark:text-white text-base">Website Engineering</div>
            <p className="text-xs text-slate-600 dark:text-slate-400">Fixed-scope $2k–$5k conversion-focused Next.js 16 websites for established local &amp; commercial firms.</p>
            <Link href="/services/websites" className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:underline">
              View Website Packages <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="p-5 rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/60 space-y-3">
            <div className="font-bold text-slate-900 dark:text-white text-base">AI Web Applications</div>
            <p className="text-xs text-slate-600 dark:text-slate-400">Custom web applications with embedded AI assistants, intelligent automation, and custom workflows.</p>
            <Link href="/services" className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline">
              Explore AI Web Apps <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="p-5 rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/60 space-y-3">
            <div className="font-bold text-slate-900 dark:text-white text-base">Secure Client Portals</div>
            <p className="text-xs text-slate-600 dark:text-slate-400">Multi-tenant client dashboards, file sharing, intake workflows, and RBAC security.</p>
            <Link href="/services" className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline">
              Explore Client Portals <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="p-5 rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/60 space-y-3">
            <div className="font-bold text-slate-900 dark:text-white text-base">AI &amp; API Integrations</div>
            <p className="text-xs text-slate-600 dark:text-slate-400">Connecting legacy tools, CRMs, database pipelines, and automated LLM endpoints.</p>
            <Link href="/services" className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline">
