...
## Active Usage Notes (Last Updated: 2024-12-14)

### Currently Active Components
✅ **FastAPI**
- Main API routes in `/api/routers/`
- Auth handling in `auth.py`
- Job processing in `jobs.py`
- Active middleware: CORS, Prometheus metrics

✅ **Firebase Admin**
- Used for authentication
- Cloud-only mode (no client SDK)
- Firestore for data storage
- Currently using Admin SDK 6.2.0

✅ **Vue.js Frontend**
- Active components in `/src/views/` and `/src/components/`
- Using Vue 3.4 composition API
...
❌ Redis (removed)
❌ Celery (removed)
❌ Express.js (installed but not used)
❌ Client-side Firebase SDK (forbidden by design)

### Pending Evaluation
⏳ Cloud Run for compute tasks
...
- **FastAPI 0.85.0**
  ...
  - Async API endpoints (/api/routers)
