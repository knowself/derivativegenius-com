...
- Decision log (append-only):
  ...
  - **2026-09-08 — Node unified on 24.20.0.** The 2026-09-06 "unify ASAP, never flag-day" condition is met: `test` (34/34) + `lint` (0 errors) + `build` + `eve eval` (3/3) all pass on Node 24.20.0. `.nvmrc` → `24.20.0`, root + agent `engines` → `24.x`, `@types/node` → `^24`, `deployment/scripts/verify.sh` floor → 24, side-by-side Node 22 retired. Rollback is revert of this change. Vercel follows `engines` (no version pin in `vercel.json`).
