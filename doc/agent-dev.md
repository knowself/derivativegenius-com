...
`dg-web` today is a Next.js 16 App Router site + `/centurion` operator app. Site and agents are unified on **Node 24.20.0** (root `.nvmrc`, root + agent `engines: 24.x`; unified 2026-09-08 after `test` + `lint` + `build` + `eve eval` all passed on Node 24 — see decision log).
...
- Upgrade path (done 2026-09-08): site + agents unified on Node 24.20.0 (`engines: 24.x` everywhere). Track in DT-08 (reproducible deploy).
...
- Node is unified on 24.20.0 repo-wide (`node`/`npm` → `~/.local/node-v24.20.0-linux-x64`, per `.nvmrc`; root + agent `engines: 24.x`). Unified 2026-09-08 after verification passed (see decision log); rollback is `git revert` of the unification commit.
...
| 2026-09-08 | Node 24 unification check | `npm test` 34/34, `npm run lint` 0 errors, `next build` clean, agent `tsc` clean both projects, `eve eval call-cta-smoke` 3/3 gates (live fallback Groq 429 → Cerebras 402 → OpenRouter pass) — all on Node 24.20.0 | Pre-unification gate evidence; lint required ignoring `agents/**/.eve`, `agents/**/.output`, `agents/**/node_modules` (2,300+ generated files were being crawled) |
| 2026-09-08 | audit-agent production deploy | `eve deploy --project dg-audit-agent` (team `derivativegenius`) → `https://dg-audit-agent.vercel.app`; first build failed (`No model provider configured` — build resolves candidates, keys must pre-exist in Vercel env) → set 4 provider keys as Production secrets → clean deploy | Keys-before-deploy is a hard prerequisite |
| 2026-09-08 | Production turn proof + env-value fix | `eve invoke --url` turn failed all-provider 401s (Groq/Cerebras/OpenRouter/Zen) while same keys worked locally → values re-added byte-clean + redeploy → `completed`, 3 findings; Vercel logs show live chain Groq 429 → Cerebras 402 → OpenRouter serve | Production $0 inference proven; free-tier *limits* are the ceiling. Value hygiene: assert key lengths before `vercel env add` |
| 2026-09-08 | dg-web → agent prod wiring | Channel trusts dg-web prod OIDC subject (`vercelSubject`); site route mints `@vercel/oidc` token on Vercel, no auth locally; `AUDIT_AGENT_URL` set on dg-web Vercel prod env (inert until site redeploys); anonymous prod turns 401 by design | Remaining switch: dg-web production redeploy |
- Decision log (append-only):
  ...
  - **2026-09-08 — audit-agent live in production.** `dg-audit-agent` (team `derivativegenius`) serves at `https://dg-audit-agent.vercel.app` with all four provider keys as Production secrets. Proven by headless `eve invoke --url` turn → `completed`, 3 findings, with the fallback chain visibly absorbing Groq 429 + Cerebras 402 in Vercel logs. dg-web calls it server-to-server via OIDC trust (route falls back to static checks on any failure); `AUDIT_AGENT_URL` is set on dg-web prod env, taking effect on the next site deploy. Optio stays local-only until its own deploy is approved.
  - **2026-09-08 — Node unified on 24.20.0.** The 2026-09-06 "unify ASAP, never flag-day" condition is met: `test` (34/34) + `lint` (0 errors) + `build` + `eve eval` (3/3) all pass on Node 24.20.0. `.nvmrc` → `24.20.0`, root + agent `engines` → `24.x`, `@types/node` → `^24`, `deployment/scripts/verify.sh` floor → 24, temporary side-by-side setup retired. Rollback is revert of this change. Vercel follows `engines` (no version pin in `vercel.json`).
